...
public class MonthYeJiListHolder extends BaseViewHolder<YeJiListBean.DataBean.YeJiBean> {
    ...
    private TextView tvName, tvOrderNum, tvDianBao, tvZiYing, tvNewly, tvActive, tvKpiTotal, tvKpiZiYing, tvTime;

    public MonthYeJiListHolder(ViewGroup parent, @LayoutRes int res) {
        super(parent, res);
        ...
        tvKpiTotal = $(R.id.tv_kpi_total_yeji);
        tvKpiZiYing = $(R.id.tv_kpi_ziying_yeji);
        tvTime = $(R.id.tv_time_month_yeji);
    }
    ...
    public void setData(YeJiListBean.DataBean.YeJiBean data) {
        ...
        tvKpiTotal.setText(StringUtil.formatNumbers(data.kpiTurnover));
        tvKpiZiYing.setText(StringUtil.formatNumbers(data.zjKpiTurnover));
