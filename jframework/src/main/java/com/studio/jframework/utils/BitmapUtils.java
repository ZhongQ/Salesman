...
//            return BitmapFactory.decodeStream(inputStream, null, options);
    ...
    public static Bitmap correctImgDegres(File file, int simpleSize) {
     ...
     * @return
     */
    public static Bitmap getPicBitmap(String imgPath, int requestWidth, int requestHeight) {
        Bitmap bitmap = null;
        try {
            Bitmap bitmapTemp = decodeSampledBitmapFromPath(imgPath, requestWidth, requestHeight);
            // 图片旋转角度
            int degree = readPictureDegree(imgPath);
            LogUtils.d("BitmapUtils", "角度" + degree);
            if (degree != 0) {
                bitmap = rotateImageView(degree, bitmapTemp);
                bitmapTemp.recycle();
            } else {
                return bitmapTemp;
            }
